...
$var( function() {
    ...
    $var("#wcmamtx_reset_download_button").on('click',function() {
        var result = confirm(wcmamtxadmin.restorealert);
        
        if (result == true) {
     
            $var.ajax({
                data: {action: "restore_my_account_download",nonce:wcmamtxadmin.nonce },
                type: 'POST',
                url: wcmamtxadmin.ajax_url,
                success: function( response ) { 
                     window.location.reload();
    ...
    $var(".checkout_field_rule_parentfield,.wcmamtx_value_select").select2({
        width: '400px'
    });
    ...
    $var("select.wcmamtx_value_select").on("change",function(event) {
        ...
        var defalt_val = $var(".wcmamtx_accordion_input_custom_key").attr("default_key");

        if (evalue == "customkey") {
            
            $var("tr.wcmamtx_customkey_tr").show();
            $var("tr.wcmamtx_checkoutfield_tr").hide();

            $var(".wcmamtx_accordion_input_custom_key").val(defalt_val);
        
        } else if (evalue == "checkoutfield") {
            $var("tr.wcmamtx_checkoutfield_tr").show();

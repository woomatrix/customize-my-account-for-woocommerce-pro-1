...
if (!class_exists('wcmamtx_add_frontend_class')) {
  ...
  class wcmamtx_add_frontend_class {
    ...
    public function __construct() {
      ...
      add_filter( 'woocommerce_account_orders_columns', array( $this, 'woocommerce_account_orders_columns_func' ),100 , 1 );
      add_filter ('woocommerce_my_account_my_orders_actions',  array( $this, 'woocommerce_my_account_my_orders_actions_func' ), 10, 2 );

      add_filter( 'woocommerce_account_downloads_columns', array( $this, 'woocommerce_account_download_columns_func' ),100 , 1 );
 ...
 */
            ...
            foreach ($new_actions as $key=>$value) {
                ...
                if (isset($value['endpoint_name']) && isset($value['action_url']) && ($value['action_url'] != "")) {
                    ...
                    foreach ($params as $skey=>$svalue) {
                        ...
                        if ($svalue == "{orderid}") {
                            $action_url = preg_replace('{orderid}', $order_id , $action_url);
                        } else {
                            ...
                            $metavalue  = get_post_meta($order_id,$svalue,true);
    ...
    public function woocommerce_account_download_columns_func($columns = array()) {

        $new_columns = (array) get_option('wcmamtx_download_columns');
    ...
    public function process_column_values($order,$column_id){
        ...
        $custom_key2 =$main_array[$column_id]['custom_key2'];
        ...
        switch($column_val_type) {
            ...
            case "customkey":
                ...
                $first_value =  wcmamtx_wpd_get_order_meta_by_order_id( $order_id, $custom_key );
            ...
            case "checkoutfield":
               ...
               $field_data = $this->wcmamtx_get_checkout_fields_data($order_id,$custom_key2);
    ...
    public function wcmamtx_get_checkout_fields_data($order_id,$custom_key2) {
        ...
        switch($custom_key2) {
            ...
            case "shipping_company":
                $data = $order->get_shipping_company();
            break;
    ...
    public function wcmamtx_override_default_navigation_template( $template, $template_name, $template_path ) {
        ...
        if (isset($plugin_options['custom_login']) && ($plugin_options['custom_login'] == "yes") ) { 

            if ( strstr($template, 'form-login.php') && ($plugin_options['custom_login_register_template'] != "default")) {

              if (class_exists("\\Elementor\\Plugin")) {
                $post_ID = $plugin_options['custom_login_register_template'];
            ...
            if ( strstr($template, 'form-lost-password') && ($plugin_options['custom_lost_password_template'] != "default")) {

              if (class_exists("\\Elementor\\Plugin")) {
                $post_ID = $plugin_options['custom_lost_password_template'];
    ...
    public function wcmamtx_load_assets() {
      ...
      if (isset($wcmamtx_plugin_options['horizontal_menu']) && ($wcmamtx_plugin_options['horizontal_menu'] == "yes")) {
        ...
        if (is_account_page()) {
		    ...
		    if ($menu_shape == "horizontal") {

                wp_enqueue_style( 'wcmamtx-frontend', ''.wcmamtx_PLUGIN_URL.'assets/css/frontend-vertical.css' );
        ...
        $version = wcmamtx_get_woo_version_number_this();
